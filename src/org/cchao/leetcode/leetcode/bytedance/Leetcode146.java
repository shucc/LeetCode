...
public class Leetcode146 {
    ...
    private static class LRUCache {
        ...
        private void changeKey(int key) {
            if (keyMap.get(nowNum) == key) {
                return;
            }
            //当前key对应的之前的nowNum数值
            int index = 1;
            for (int item : keyMap.keySet()) {
                if (keyMap.get(item) == key) {
                    index = item;
                    break;
                }
            }
            for (int i = index + 1; i <= nowNum; i++) {
    ...
    private static void process(String[] command, int[][] nums) {
        LRUCache lruCache = new LRUCache(nums[0][0]);
        for (int i = 1; i < command.length; i++) {
            if (command[i].equals("put")) {
                lruCache.put(nums[i][0], nums[i][1]);
            } else {
                System.out.print(lruCache.get(nums[i][0]) + ",");
            }
        }
    }
}
